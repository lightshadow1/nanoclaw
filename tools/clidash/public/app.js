...
async function fetchJson(url) {
  try {
    ...
    return { ok: false, error: 'Dashboard unavailable; showing the last successful snapshot if present.' };
  }
}
...
async function refresh(force = false) {
  ...
  state.connectionError = cliList?.clis ? null : 'Dashboard connection unavailable. Last successful snapshots may be stale.';
  ...
  if (cliList?.clis && ![...state.errors.values()].some(Boolean)) state.lastUpdated = new Date();
...
function dataSignature() {
  ...
  return JSON.stringify({
    ...
    fetchedAt: key ? state.snapshots.get(key)?.fetchedAt ?? null : null,
    ...
    overview: v.type === 'overview' ? {
      forkRows: [...state.snapshots.entries()], forkErrors: [...state.errors.entries()],
...
function render() {
  ...
  const bannerMsg = state.connectionError || (cli?.error ? `Discovery failed for ${v.cli}: ${cli.error}`
    : (tabError ? `Data unavailable. ${tabError.split('\n')[0]}` : null));
...
function renderCmdline(v) {
  const bar = $('cmdline');
  let cmd = null;
  if (v.type === 'resource' && v.cli !== 'nanoclaw') cmd = state.snapshots.get(`${v.cli}/${v.resource}`)?.command;
  ...
  else if (v.type === 'overview') cmd = state.clis.some((c) => c.name === 'nanoclaw') ? '' : 'derived · ncl groups/sessions/messaging-groups/wirings + config-get + activity';
  bar.hidden = !cmd;
...
function renderOverviewPage() {
  ...
  const fork = state.clis.find((c) => c.name === 'nanoclaw');
  if (fork) {
    const cards = fork.resources.map(({ name }) => {
      const key = `nanoclaw/${name}`;
      const snapshot = state.snapshots.get(key);
      const error = state.errors.get(key);
      const rows = snapshot?.rows;
      const statusKey = fork.summary?.[name];
      const counts = new Map();
      ...
      return el('div', { class: 'ov-card' }, [
        el('h3', {}, name),
        error ? el('div', { class: 'tab-error' }, error) : null,
        el('p', {}, rows ? `${rows.length}${rows.length === 500 ? '+' : ''} rows in this snapshot` : error ? 'Unavailable' : 'Loading…'),
        el('p', {}, [...counts].map(([key, count]) => `${key}: ${count}`).join(' · ')),
        el('p', { class: 'dim' }, snapshot ? `Collected ${new Date(snapshot.fetchedAt).toLocaleString()}` : 'No successful collection yet'),
        el('button', { onclick: () => go(`r:nanoclaw:${name}`) }, 'Inspect'),
      ]);
    });
    content.replaceChildren(
      el('h2', { class: 'page-title' }, 'Operations overview'),
      el('p', {}, 'Read-only · refreshes every 60 seconds · up to 500 rows per view'),
      el('p', { class: 'dim' }, 'Claimed means reserved by the scheduler, not confirmed running. Container state is collected separately. Counts describe the bounded snapshot, not all historical records.'),
...
function renderTable(cliName, resource) {
  ...
  if (cliName === 'nanoclaw') parts.push(el('p', { class: 'dim' }, snapshot ? `Last successful collection: ${new Date(snapshot.fetchedAt).toLocaleString()} · up to 500 rows` : 'No successful collection yet'));
