...
describe('GroupQueue', () => {
  ...
  it('ignores the task ID that is currently running', async () => {
    let resolveTask: () => void;
    ...
    queue.enqueueTask('group1@g.us', 'task-1', first);
    await vi.advanceTimersByTimeAsync(10);
    queue.enqueueTask('group1@g.us', 'task-1', duplicate);
  ...
  it('allows the same task ID again after the prior run finishes', async () => {
